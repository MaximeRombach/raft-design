...
focal_surfaces = {
    ...
    'DESI':
        ...
        'tilt2loss_deprecated': Polynomial([0, -0.0005305, 0.031581, -0.030215, 0.061497, -0.051610, 0.017785]),  # input units mm, included here for completeness, but these coefficients (generated by MS Excel chart in DESI-0347) are poor at reproducing the tabular data there
...
for surf in focal_surfaces.values():
    ...
    surf['defocus2blur'] = lambda dz_mm: (dz_mm*1000) / 2 / surf['f-number'] / 3  # input and output units: mm
    for key in ['blur2loss', 'tilt2loss']:
        if key not in surf or surf[key] == 'DESI-like':
            surf[key] = focal_surfaces['DESI'][key]
...
if userargs.trillium_spacing_shift:
    tuned_pitch = 6.2 # robot pitch value at which adjustments were hand-tuned, referencing CAD model
...
for j in rng:
    ...
    if userargs.trillium_spacing_shift:
        x = [u - spacing_x_adjust*j for u in x]
    ...
    y = [v + spacing_y/3 for v in y]
    if userargs.trillium_spacing_shift:
        y = [v + spacing_y_adjust for v in y]
    natural_grid['y'] += y
...
for i, raft in enumerate(rafts):
    ...
    def calc_err_or_loss(offsets):
        focus0 = raft.focus_offset
        tilt0 = raft.tilt_offset
        if optimize_focus:
            raft.focus_offset = offsets[idx_focus]
        if optimize_tilt:
            raft.tilt_offset = offsets[idx_tilt]
        focus_errors, tilt_errors = calc_focus_and_tilt_errors(raft)
        if loss_functions_are_defined:
            defocus_losses = defocus2loss(focus_errors)
            tilt_losses = tilt2loss(tilt_errors)
            throughput = (1 - defocus_losses) * (1 - tilt_losses)
        ...
        return output
    result = optimize.least_squares(fun=calc_err_or_loss, x0=offsets0)
    if optimize_focus:
...
if loss_functions_are_defined:
    defocus_losses = defocus2loss(robots['focus error'])
